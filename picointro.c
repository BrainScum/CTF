//c can allow direct control of machines resources
//systems programming requires extremely efficient code, therefore often being programmed in c

//compiling is just the process of translating a source code into machine code (binary)

//%c to output a char - %x for each respective data type
...
int values() {
    ...
    printf("\n my float: %f ", c);
    printf("\n my double: %.16g \n", d);
    printf("\n my char %c , my second char %c , my third char %c  ",e,f,g);
    return 0;
}

//x to create a variable 
//*y to create a pointer
//y = &x to assign that pointer to the value x

//char c = "Q" to declare a variable and assign it a value
//char *p to create a pointer
//p=&c to assign the *address* of char c to pointer p

//printing c or *p outputs Q because c is the value and *p is the *value of the address* that p is pointing to 
//printing &c or p outputs the pointer value? because &c is *the address* and p is *the address that the pointer points to*
...
/*
Data section: static and global variables
Heap: memory allocated dynamically, malloc in c allocates a buffer - it is placed on the heap
Stack: local variables, function parameters, return addresses (where to return after function is 
 finished) - stack pointer for return address sits at the top of the stack
Registers: similar to memory and attached to processor - execution of a program and its memory,
 can hold 4 (32 bit) or 8 (64 bit) bytes, instruction pointer (IP) keeps track of the currently executed
 program, it can be accessed or printed in gdb with x $rip
little endian: computers interpret machine code as little endian (backwards?)
*/